...
int Shader::GetUniformLocation(const std::string& name)
{
	...
	if (location == -1)
		std::cout << "Warning: unifor '" << name << "' doesn't exist!" << std::endl;
...
unsigned int Shader::CompileShader(unsigned int type, const std::string& source)
{
	...
	// Handle shaders compilation errors
	int result;
	...
	if (result == GL_FALSE)
	{
		...
		// Nice way to create an array on the stack with the size of a variable (instead of "char message[length];")
